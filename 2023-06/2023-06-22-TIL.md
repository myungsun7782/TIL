...
- **⭐️ 요청(Request) --> 서버 데이터(JSON) --> 분석(Parse) --> 변환(우리가 쓰려는 struct/class) ⭐️**
- 아래 예제는 영화 진흥위원회 오픈 API를 사용해서 특정 일자 상영작들의 박스오피스 정보를 조회하는 코드입니다.
- 참고 URL
  ...
  - http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=KEYVALUE&targetDt=20211201 (요청 예시)
